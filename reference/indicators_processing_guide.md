...
This guide provides detailed specifications for processing all 38 household survey indicators from the Tawila WASH Assessment, aligned with **Annex 1: Household Survey Indicators** from the Terms of Reference.
...
- **Table 2**: WASH Theme Indicators (29 indicators)
  - Water: 7 indicators (1.1-1.6, 1.9)
  - Sanitation: 10 indicators (2.1-2.9, 3.0)
  - Hygiene: 10 indicators (4.1-4.7.2, 4.8)
  - Priorities: 1 indicator (5.1)
...
| Respondent gender | Gender of survey respondent | `gender_of_the_respondent` | select_one | Overall + by camp | Demographics question | Dot plot with confidence intervals |
| Gender/age of head of household | Gender and age of HoH | `is_the_responder_head_of_the_household`, `if_no_please_specify_the_age_of_the_household_head`, `specify_the_gender_of_the_househld` | mixed (select_one + integer) | Overall + by camp | If respondent is HoH, use their gender/age. Otherwise use specified HoH gender/age | Grouped bar chart (gender) + histogram (age distribution) |
| Recently arrived households | HH arrived within past 2 weeks | `did_people_arrive_two_weeks_ago_into_tawila` | select_one | Overall + by camp | Binary yes/no | Dot plot with confidence intervals |
| Households with members under 5 years old | HH with children <5 | `do_you_have_members_less_than_5_years_old` | select_one | Overall + by camp | Binary yes/no | Dot plot with confidence intervals |
| Households with elderly members (over 60) | HH with members 60+ | `no_of_men_60_in_hh`, `no_of_women_60_in_hh` | integer | Overall + by camp | Derive binary: `(no_of_men_60_in_hh + no_of_women_60_in_hh) > 0` | Dot plot with confidence intervals |
| Households with members with disabilities | HH with disabled members | `no_of_people_with_disabilities_in_hh_optional` | integer | Overall + by camp | Derive binary: `no_of_people_with_disabilities_in_hh_optional > 0` | Dot plot with confidence intervals |
| Households with pregnant or lactating women | HH with PLW | `do_you_have_members_with_pregnant_or_lactating_women` | select_one | Overall + by camp | Binary yes/no | Dot plot with confidence intervals |
| Households with child in malnutrition treatment | HH with child receiving treatment | `do_you_have_members_with_child_that_is_currently_receiving_malnutrition_treatment` | select_one | Overall + by camp | Binary yes/no | Dot plot with confidence intervals |
...
- **4.7.1, 4.7.2**: Split from original 4.7 in TOR revision - separate soap availability from barriers
- **4.8**: Respondent age used as proxy; document this limitation clearly

### Priorities (1 indicator)

| Indicator | Description | Kobo Field(s) | Question Type | Aggregation | Processing Notes | Graphical Output |
|-----------|-------------|---------------|---------------|-------------|------------------|------------------|
| 5.1 | % of HH by main priority concerns reported | `hh_h_6_1_which_of_the_following_is_your_biggest_wash_related_concern_right_now_for_your_household` | select_one | Overall + by camp | Categories: water access, water quality, sanitation access, sanitation quality, hygiene items, solid waste management, other | Stacked bar chart showing distribution of priority concerns |

**Priorities processing notes:**
- Single priority question captures overall WASH concern ranking
- Can cross-tabulate with disaggregation variables to understand differential priorities by subgroup
...
- [✓] Exactly 38 indicators in Tables 1 and 2
- [✓] All TOR indicator numbers present (1.1-1.9, 2.1-2.9, 3.0, 4.1-4.8, 5.1)
- [✓] Indicators 4.7.1 and 4.7.2 correctly separated (soap at home vs. soap barriers)
- [✓] Indicator 4.8 includes respondent age disaggregation note
...
- [✓] Processing notes address special cases:
  ...
  - [✓] 4.8: Respondent age as proxy for menstruating individuals
